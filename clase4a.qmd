...
eph_ind <- eph_ind %>%
  mutate(
    # Si hay un valor NA en PP04D_COD, se toma el valor de PP11D_COD, de lo contrario se toma el valor de PP04D_COD.
    cno = case_when(is.na(PP04D_COD) ~ PP11D_COD,
                    TRUE ~ PP04D_COD),
    cno12 = case_when(nchar(cno) > 4 ~ str_sub(cno, 1, 2),
                      TRUE ~ str_sub(cno, 1, 1)),
    cno3 = case_when(nchar(cno) > 4 ~ str_sub(cno, 3, 3),
                     TRUE ~ str_sub(cno, 2, 2)),
    cno4 = case_when(nchar(cno) > 4 ~ str_sub(cno, 4, 4),
                     TRUE ~ str_sub(cno, 3, 3)),
    cno5 = case_when(nchar(cno) > 4 ~ str_sub(cno, 5, 5),
                     TRUE ~ str_sub(cno, 4, 4)),
    # pasar a numérico
...
- *Calidad de representación*: es el coseno cuadrado de la distancia entre la modalidad y el eje, es decir, indica qué tan bien está representada una categoría en el espacio factorial considerado.  
