...
eph_seleccion <- eph_ind %>% 
  ...
  select(jerarquia, calificacion, sexo, edad_cat, nivel_educativo, situacion_conyugal, decil_IPCF, decil_ITI, PONDERA, uso_ahorros, prestamos_familiares, prestamos_financieros, auh)  
```
...
ggadd_supvars(nube_v, mca, vars = eph_seleccion[,10:13]) # agregamos la capa de variables suplementarias
...
```{r}
dimeta2(mca, eph_seleccion[,10:13])
...
```{r}
suplementarias <- dimdescr(mca, eph_seleccion[,10:13])
...
```{r}
dimtypicality(mca, eph_seleccion[,10:13], dim = c(1,2))
```
